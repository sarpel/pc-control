    ...
    async def send_message(self, message: Dict[str, Any], compress: bool = True) -> bool:
        ...
        try:
            ...
            if len(self.send_buffer) >= 100:
                logger.warning(
                    f"Send buffer full for device {self.device_id}, dropping oldest message"
                )
            ...
            try:
                if self.send_buffer:
                    ...
                    for message_bytes in messages_to_send:
                        if isinstance(message_bytes, bytes) and message_bytes.startswith(
                            b"\x1f\x8b"
                        ):
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        # Register connection
        if not connection_manager.register_connection(
        ...
        await handler.send_message(
            ...
            compress=False,
        )  # Don't compress the initial message
...
async def handle_websocket_messages(
    websocket: WebSocket, device_id: str, device_info: Dict[str, Any]
):
    """
        ...
        while True:
            ...
            if message_type == "audio_data":
                ...
                await self._send_message(
                    websocket, {"type": "pong", "timestamp": get_current_timestamp()}
                )
            ...
            json.dumps(
                {
                    ...
                    "status": "completed",
                    "parameters": {"application_name": "Chrome"},
                    "result": {
                        "success": True,
...
async def handle_audio_data_optimized(
    handler: OptimizedWebSocketHandler, device_id: str, message_data: Dict[str, Any]
):
    """
    ...
    try:
        ...
        raw_audio = audio_chunk.encode("latin1") if isinstance(audio_chunk, str) else audio_chunk
        processed_chunk = handler.audio_processor.process_audio_chunk(raw_audio)

        # Accumulate audio for STT
        handler.stt_buffer.write(raw_audio)
        ...
        if is_final:
            ...
            audio_data = handler.stt_buffer.getvalue()
            handler.stt_buffer = io.BytesIO()  # Reset buffer
            ...
            if stt_result.success and stt_result.text:
                ...
                # Execute command
                command_result = await voice_processor.process_voice_command(
                    transcription=transcription, confidence=confidence, device_id=device_id
                )
                ...
                await handler.send_message(
                    {
                        "type": "action_execution",
                        "device_id": device_id,
                        "action_id": command_result.command_id,
                        "action_type": "compound",  # Could be multiple actions
                        "status": "completed" if command_result.success else "failed",
                        "result": {
                            "success": command_result.success,
                            "message": command_result.response_message,
                            "execution_time_ms": command_result.execution_time_ms,
                            "action_results": command_result.action_results,
                        },
                        "timestamp": datetime.utcnow().isoformat(),
                    },
                    compress=True,
                ...
                await handler.send_message(
                    {
                        "type": "command_status",
                        "device_id": device_id,
                        "status": "tamamlandı" if command_result.success else "hata",
                        "timestamp": datetime.utcnow().isoformat(),
                    }
                )
            else:
                error_msg = stt_result.error or "Ses anlaşılamadı"
                await handler.send_message(
                    {
                        "type": "error",
...
async def handle_voice_command_optimized(
    handler: OptimizedWebSocketHandler, device_id: str, message_data: Dict[str, Any]
):
    """
    ...
    try:
        ...
        await handler.send_message(
            ...
            compress=True,
        )  # Use compression for this larger message
...
async def handle_wake_on_lan_optimized(
    handler: OptimizedWebSocketHandler, device_id: str, message_data: Dict[str, Any]
):
    """
